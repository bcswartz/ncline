...
describe( 'core', function() {
    ...
    describe( 'getModulePath', function() {
        ...
        it( 'should return the last value of a backslash-separated filepath if the os is win32', function() {
            platformStub = sinon.stub( os, 'platform', function() { return 'win32'; } );
            var result = core.getModulePath( 'C:\\Windows\\storage' );
            expect( result ).to.equal( 'storage' );
        });

        it( 'should return the last value of a slash-separated filepath if the os is not win32', function() {
            platformStub = sinon.stub( os, 'platform', function() { return 'darwin'; } );
            var result = core.getModulePath( '/Users/Bob/Documents/util' );
    ...
    describe( 'createTerminalExecutionPrefix', function() {
        ...
        describe( 'when os.platform() returns "darwin"', function() {
            it( 'will return a statement starting with "open -a Terminal"', function() {
                platformStub = sinon.stub( os, 'platform', function() { return 'darwin'; } );
                var result =  core.createTerminalExecutionPrefix( '/Users/Bob/Documents' );
                expect( result ).to.match( /^open \-a Terminal/ );
            });

            it( 'will include the path starting with a double-quote', function() {
                platformStub = sinon.stub( os, 'platform', function() { return 'darwin'; } );
                var result =  core.createTerminalExecutionPrefix( '/Users/Bob/Documents' );
                expect( result ).to.equal( 'open -a Terminal "/Users/Bob/Documents' );
            });
        ...
        describe( 'when os.platform() returns something other than "win32" or "darwin"', function() {
           it( 'will throw an error', function() {
               platformStub = sinon.stub( os, 'platform', function() { return 'linux'; } );
               expect( function() { core.createTerminalExecutionPrefix( 'C:\\filePath', 'windowTitle' ) } ).to.throw( Error, /Currently only executes on Windows and Macintosh operating systems./ );
