...
describe( 'core', function() {
    ...
    describe( 'generateCommandPrompt', function() {
        ...
        var emptySourceFn, sourceFn, targetFn;

        before( function() {
            emptySourceFn = function() {};
            sourceFn = function() { return 'srcDir' };
            targetFn = function() { return 'targetDir' };
        })

        it( 'should start with a source alias if a source is set', function() {
            var cmd = { getSourceAlias: { function: sourceFn }, getTargetAlias: { function: targetFn} };
            var result = core.generateCommandPrompt( cmd );
            expect( result ).to.match( /^source \-\> srcDir/ );
        });

        it( 'should have no source if no source is set', function() {
            var cmd = { getSourceAlias: { function: emptySourceFn }, getTargetAlias: { function: targetFn} };
            var result = core.generateCommandPrompt( cmd );
        ...
        it( 'should always include the target directory at the end', function() {
            ...
            var cmdNoSource = { getSourceAlias: { function: emptySourceFn }, getTargetAlias: { function: targetFn} };
            var resultNoSource = core.generateCommandPrompt( cmdNoSource );
