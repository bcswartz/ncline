...
//Deferring load of output and fp module until end of file to resolve circular dependency
...
module.exports = {
    ...
    getModulePath: function( dirname ) {
        if( os.platform() == 'win32' ) {
            return dirname.split( '\\' )[ ( dirname.split( '\\' ).length - 1 ) ];
        } else {
            return dirname.split( '/' )[ ( dirname.split( '/' ).length - 1 ) ];
        }
    },

    generateCommandPrompt: function() {
        var promptText;
        if( fp.hooks.getSourceAlias() != undefined ) {
            promptText = 'source -> ' + fp.hooks.getSourceAlias() + ', ';
        } else {
            promptText = '';
        }
        promptText += 'target -> ' + fp.hooks.getTargetAlias() + ' >>';
        return promptText;
    },
    ...
    sortObjectProperties: function( obj ) {
        ...
        var objectWithSortedProperties =  _.zipObject( keys, _.map( keys, function ( key ) {
            return obj[ key ];
        }));
...
//Load any need core command modules
var fp = require( '../cmdModules/core/filePath/commands' );
